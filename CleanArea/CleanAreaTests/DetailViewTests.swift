...
final class DetailViewTests: XCTestCase {
    let itemModelContext: ModelContext = {
        let schema = Schema([
            YouthPolicy.self,
        ])
    ...
    func testOnAppear() async {
        let store = TestStore(initialState: DetailFeature.State(
            youthPolicy: mok2
        )) {
            DetailFeature()
        } withDependencies: {
        ...
        await store.send(.onAppear) {
            $0.likePolicies = IdentifiedArray(uniqueElements: self.mok1)
        }
        
        await store.receive(\.checkLikeStatus)
        
        await store.receive(\.setStatus) {
            $0.likeStatus = true
        }
    }
    
    func testTabLikeButton() async {
        let store = TestStore(initialState: DetailFeature.State(
            likePolicies: [],
            youthPolicy: mok2
        )) {
            ...
            $0.databaseService.context = {
                self.itemModelContext
            }
