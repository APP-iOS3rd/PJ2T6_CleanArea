...
final class ListViewTests: XCTestCase {
    ...
    /*
     ...
     case setText(String)
     */
    func testClearTextField() async {
        let store = TestStore(initialState: ListFeature.State(
            filteredPolicies: IdentifiedArray(uniqueElements: mok1),
            hearderTitle: "추천정책",
    ...
    func testFilterPolicies() async {
        let store1 = TestStore(initialState: ListFeature.State(
            filteredPolicies: [],
            hearderTitle: "추천정책",
            policies: IdentifiedArray(uniqueElements: self.mok1),
            tabType: .recommand,
            text: ""
        )) {
            ListFeature()
        ...
        let store2 = TestStore(initialState: ListFeature.State(
            filteredPolicies: IdentifiedArray(uniqueElements: self.mok1),
            hearderTitle: "추천정책",
            policies: IdentifiedArray(uniqueElements: self.mok1),
            tabType: .recommand,
            text: "asdasd"
        ...
        let store3 = TestStore(initialState: ListFeature.State(
            filteredPolicies: [],
            hearderTitle: "추천정책",
            policies: IdentifiedArray(uniqueElements: self.mok1),
            tabType: .recommand,
        ...
        await store1.send(.filterPolicies("")) {
            $0.filteredPolicies = IdentifiedArray(uniqueElements: self.mok1)
        }
        
        await store2.send(.filterPolicies("asdasd")) {
            $0.filteredPolicies = []
        }
        
        await store3.send(.filterPolicies("수영구")) {
            $0.filteredPolicies = IdentifiedArray(uniqueElements: self.mok1)
        }
    }
}
