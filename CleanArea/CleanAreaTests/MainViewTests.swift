...
final class MainViewTests: XCTestCase {
    func testOnAppear() async {
        ...
        let store = TestStore(initialState: MainFeature.State(
            policies: IdentifiedArray(uniqueElements: mok1)
        )) {
            MainFeature()
        } withDependencies: {
            $0.swiftData.fetch = {
        ...
        await store.send(.appearSet) {
            $0.likePolicies = IdentifiedArray(uniqueElements: mok1)
        }
        
        await store.receive(\.setFilterPolicy) {
            $0.hotPolicies = IdentifiedArray(uniqueElements: mok1)
        }
        
        await store.receive(\.getPolicy)
        await store.receive(\.initialAppear) {
            $0.initialAppear = false
        }
        
        await store.receive(\.filterPolicy)
        await store.receive(\.setRecommanModel) {
            $0.recommandcellModels = [
                RecommandCellModel(name: "일자리", count: 0, destinationKey: "일자리", policies: [])
            ]
        }
        ...
        await store.receive(\.setRecommanModel) {
            $0.recommandcellModels = [
                RecommandCellModel(name: "일자리", count: 0, destinationKey: "일자리", policies: []),
                RecommandCellModel(name: "주거", count: 1, destinationKey: "주거", policies: IdentifiedArray(uniqueElements: mok1))
            ]
        }
        ...
        await store.receive(\.setRecommanModel) {
            $0.recommandcellModels = [
                RecommandCellModel(name: "일자리", count: 0, destinationKey: "일자리", policies: []),
                RecommandCellModel(name: "주거", count: 1, destinationKey: "주거", policies: IdentifiedArray(uniqueElements: mok1)),
                RecommandCellModel(name: "교육", count: 0, destinationKey: "교육", policies: [])
            ]
        }
        ...
        await store.receive(\.setRecommanModel) {
            $0.recommandcellModels = [
                ...
                RecommandCellModel(name: "주거", count: 1, destinationKey: "주거", policies: IdentifiedArray(uniqueElements: mok1)),
                RecommandCellModel(name: "교육", count: 0, destinationKey: "교육", policies: []),
                RecommandCellModel(name: "복지, 문화", count: 0, destinationKey: "복지, 문화", policies: [])
            ]
        }
        ...
        await store.receive(\.setRecommanModel) {
            $0.recommandcellModels = [
                ...
                RecommandCellModel(name: "교육", count: 0, destinationKey: "교육", policies: []),
                RecommandCellModel(name: "복지, 문화", count: 0, destinationKey: "복지, 문화", policies: []),
                RecommandCellModel(name: "참여, 권리", count: 0, destinationKey: "참여, 권리", policies: [])
            ]
        }
    }
}
